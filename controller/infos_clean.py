...
class InfosClean:
    """
    Classe responsável pela limpeza e preparação de dados de proposições e tramitações.

    A classe processa os resultados da API para criar DataFrames que representam as tabelas de proposições e tramitações.

    Atributos:
        df_proposicao_initial (pd.DataFrame): DataFrame inicial de proposições após limpeza e formatação.
        df_tramitacao_initial (pd.DataFrame): DataFrame inicial de tramitações após limpeza e formatação.

    Métodos:
        __init__(infos_api_results): Inicializa a classe com os resultados da API e cria os DataFrames iniciais.
        _create_first_df_proposicao(api_results): Cria e limpa o DataFrame de proposições.
        _create_first_df_tramitacao(api_results): Cria e limpa o DataFrame de tramitações.
    """

    def __init__(self, infos_api_results) -> None:
        """
        Inicializa a classe InfosClean e cria os DataFrames iniciais de proposições e tramitações.

        Parâmetros:
            infos_api_results (list): Lista de resultados da API contendo dados de proposições e tramitações.
        """
    ...
    def _create_first_df_proposicao(self, api_results):
        """
        Cria e limpa o DataFrame de proposições a partir dos resultados da API.

        Parâmetros:
            api_results (list): Lista de resultados da API contendo dados de proposições.

        Retorna:
            pd.DataFrame: DataFrame limpo e formatado de proposições.
        """
        ...
        df_proposicao = pd.DataFrame(data_for_dataframe, columns=list_keys_proposicao) # Cria o DataFrame
        ...
        dtype_mapping = { # Faz o mapeamento de tipos para converter colunas no DF
            'number': 'str',
            'year': 'int',
            'author': 'str',
            'situation': 'str',
    ...
    def _create_first_df_tramitacao(self, api_results):
        """
        Cria e limpa o DataFrame de tramitações a partir dos resultados da API.

        Parâmetros:
            api_results (list): Lista de resultados da API contendo dados de tramitações.

        Retorna:
            pd.DataFrame: DataFrame limpo e formatado de tramitações.
        """
